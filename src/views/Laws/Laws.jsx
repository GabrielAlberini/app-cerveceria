import { Layout } from "../../components/Layout/Layout";
import { Banner } from "../../components/Banner/Banner";
import "./Laws.css";

export const Laws = () => {
  return (
    <Layout>
      <Banner
        title="BASES Y CONDICIONES PROMOCION: “Mes del Liso 2023”."
        urlImage={"./fabrica.jpg"}
      />
      <section className="p-6 cont-laws-page">
        <div className="pt-5 pb-5">
          <h2 className="num-law">1.</h2>
          <p>
            Compañía Industrial Cervecera S.A.(en adelante el “Organizador”),
            CUIT Nº 30-50577985- 8, con domicilio en Calchines 1401, (CP 3000)
            Santa Fe, provincia de Santa Fe, implementa la promoción denominada
            “Mes del Liso” (en adelante la “Promoción”) que se regirá por lo
            establecido en estas bases y condiciones (en adelante las “Bases”)
            copia de las cuales podrán consultarse en el siguiente link:
            www.lisosantafesino.com.ar
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law">2.</h2>
          <p>
            La participación en la Promoción implica el conocimiento y la
            aceptación total e incondicional de las presentes Bases.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law">3.</h2>
          <p>
            <strong>Validez y Vigencia:</strong> La Promoción será válida
            exclusivamente en la Ciudad de Santa Fe y Santo Tomé, Provincia de
            Santa Fe, Argentina (en adelante el “Territorio”), y tendrá vigencia
            desde las 00:00 horas del 4 de diciembre de 2023 hasta las 23.59 del
            21 de diciembre de 2023 (en adelante el “Plazo de Vigencia” y
            resultará aplicable única y exclusivamente los días lunes, martes,
            miércoles y jueves en el horario de 19:00 hs a 21:30hs durante el
            Plazo de Vigencia. Este Plazo de Vigencia es improrrogable, no
            aceptándose en consecuencia participaciones posteriores a la
            finalización del Plazo de Vigencia de la Promoción.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law">4.</h2>
          <p>
            <strong>Participantes:</strong> Podrán participar en la Promoción
            las personas humanas, mayores de 18 años, que se encuentren en el
            Territorio, que cumplan con el mecanismo de participación indicado
            en estas Bases (en adelante el “Participante” o los “Participantes”
            según corresponda).
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 5.</h2>
          <p>
            5. Cada Participante sólo podrá participar una vez por día durante
            del Plazo de Vigencia, no aceptándose la participación bajo nombres
            o datos falsos. Está prohibido y será anulado cualquier intento o
            método de participación que se realice por cualquier proceso,
            técnica o mecánica de participación distinta a la detallada en estas
            Bases. La utilización de técnicas de naturaleza robótica,
            repetitiva, automática, programada, mecanizada o similar llevará a
            la anulación de todos los registros realizados por el Participante.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 6.</h2>
          <p>
            El Organizador se reserva el derecho de eliminar a cualquier
            Participante que no respete los términos y condiciones establecidos
            en las presentes Bases.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 7.</h2>
          <p>
            <strong>Modo de Participación:</strong> Para participar en la
            Promoción los Participantes deberán, durante el Plazo de Vigencia,
            en los días y horarios expresamente indicados en estas Bases: (i)
            Escanear con un teléfono móvil el código QR que se encontrará
            exhibido en las mesas de los puntos de venta on premise del
            Territorio adheridos a la Promoción según detalle establecido en el
            Anexo I de las presentes Bases (en adelante el “Punto de Venta
            Adherido” o “los Puntos de Venta Adheridos” según corresponda) que
            dirigirá a los Participantes al link:
            http://www.lisosantafesino.com.ar/ (la “Landing”); (ii) Registrar de
            forma manual los datos personales allí requeridos y seleccionar el
            Punto de Venta Adherido en el que se encuentre presente para
            participar en la Promoción (en adelante los “Datos Personales”);
            (iii) solicitar a algún trabajador del Punto de Venta Adherido en el
            que se encuentra presente para la obtención del Beneficio que valide
            los datos a través de la Landing. Al cumplir con la mecánica
            dispuesta en los puntos (i) a (iii) precedentes, los Participantes
            tendrán derecho a la obtención del Beneficio conforme se define en
            la Cláusula 9, el cual será acreditado automáticamente en la
            Landing.
          </p>
        </div>

        <div className="pt-5 pb-5">
          <h2 className="num-law"> 8.</h2>
          <p>
            <strong>Otorgamiento del Beneficio:</strong> Será condición
            ineludible para acceder al Beneficio el cumplimiento por cada
            Participante de los siguientes requisitos: (i) la acreditación de
            identidad y de la mayoría de edad; (ii) el cumplimiento de todas y
            cada una de las condiciones establecidas en estas Bases.
          </p>
          <p>
            Si por alguna razón no imputable al Organizador, alguno de los
            Participantes se vieran imposibilitados de recibir y/o utilizar el
            Beneficio obtenido, el Participante perderá el mismo y nada tendrá
            que reclamar al Organizador, ni se realizará reembolso o cambio
            alguno.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 9.</h2>
          <p>
            <strong>El Beneficio:</strong> Los Participantes que hubieran
            cumplido con todos y cada uno de los requisitos establecidos en las
            presentes Bases recibirán un (1) un código canjeable por una
            promoción de 5x3 cervezas calibre 250cc para su consumo en el Punto
            de Venta Adherido, es decir, la entrega de dos (2) cervezas sin
            cargo con la compra de tres (3) (en adelante el “Beneficio” según
            corresponda). El Beneficio podrá ser usado exclusivamente el día en
            que fuera otorgado dentro del horario establecido en la Cláusula 2.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 10.</h2>
          <p>
            <strong>Alcance del Beneficio:</strong> El Beneficio no incluye otro
            bien, prestación o servicio adicional no enumerado en estas Bases.
            Los Participantes, no podrán negociar ni exigir un bien distinto al
            indicado, ni por su valor en dinero. Los Beneficios son
            intransferibles y no incluyen ninguna prestación, pago, bien o
            servicio distinto de los indicados en estas Bases y los
            Participantes no podrán solicitar su sustitución por otros bienes o
            servicios.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 11.</h2>
          <p>
            <strong>Alcance del Beneficio:</strong> El Beneficio no incluye otro
            bien, prestación o servicio adicional no enumerado en estas Bases.
            Los Participantes, no podrán negociar ni exigir un bien distinto al
            indicado, ni por su valor en dinero. Los Beneficios son
            intransferibles y no incluyen ninguna prestación, pago, bien o
            servicio distinto de los indicados en estas Bases y los
            Participantes no podrán solicitar su sustitución por otros bienes o
            servicios.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 12.</h2>
          <p>
            <strong>Impuestos:</strong> Todo impuesto, tasa o contribución que
            deba tributarse, toda suma de dinero que deba pagarse al estado
            nacional, sociedades del estado, provincias o municipalidades por el
            hecho del ofrecimiento, asignación o entrega del Código de Descuento
            o con motivo de la organización o difusión de la Promoción será a su
            exclusivo cargo, a excepción de los expresamente asumidos por el
            Organizador en estas Bases.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 13.</h2>
          <p>
            <strong>Uso de datos:</strong> Consulte nuestra Política de
            Privacidad en https://www.ccu.com.ar/politica- de-privacidad/ para
            obtener información sobre cómo recopilamos, utilizamos y compartimos
            sus datos personales.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 14.</h2>
          <p>
            <strong>Garantías y Responsabilidad:</strong> El Organizador y/o sus
            accionistas, y/o directores, y/o gerentes, y/o funcionarios y/o su
            personal no serán responsables: (i) por ningún daño o perjuicio, de
            cualquier tipo que fuere, que pudieren sufrir los Participantes y/o
            terceros, en sus personas o bienes, con motivo de, o con relación a,
            su participación en la Promoción ; ni (ii) por fallas en la redes
            telefónicas o servicios de internet, desperfectos técnicos, errores
            humanos o acciones deliberadas de terceros que pudieran interrumpir
            o alterar el normal desarrollo de la Promoción. No se aceptarán
            reclamos de los Participantes por problemas de conexión en los
            equipos de informática que imposibiliten la participación de acuerdo
            a las condiciones establecidas en las presentes Bases.
          </p>
          <p>
            Los Participantes por el sólo hecho de participar en la Promoción
            garantizan: (i) que son únicos y exclusivos responsables por los
            Datos Personales y (ii) que mantendrán indemne al Organizador de las
            consecuencias de cualquier reclamo que estos pudieran recibir con
            relación a o con motivo de la información de tales Datos Personales.
            Serán excluidos de la Promoción aquellos Participantes que hubieren
            proporcionado datos falsos, inexactos y/o incompletos. El
            Organizador podrá prohibir la participación en la Promoción como así
            también descalificar y no otorgar el Código de Descuento a las
            personas que intenten hacer trampas, fraudes, engaños o cualquier
            otra práctica ilegítima.
          </p>
          <p>
            El Organizador no tiene control ni responsabilidad alguna sobre
            medidas restrictivas que pudieren dictarse a nivel nacional y/o
            internacional consecuencia de la existencia de endemias, epidemias,
            pandemias, guerras, catástrofes naturales, etc y/o cualquier otra
            restricción de hecho, por lo que no será responsable por ninguna
            modificación y/o suspensión y/o cancelación de la presente Promoción
            como así tampoco de medidas o acciones que deba cumplir el
            Participante que tengan causa en resoluciones dictadas por
            autoridades nacionales y/o internacionales.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 15.</h2>
          <p>
            <strong>Difusión:</strong> La Promoción será difundida mediante
            material publicitario exhibido en los Puntos de Venta Adheridos, y
            publicidad digital, via pública, radio y TV.
          </p>
        </div>
        <div className="pt-5 pb-5">
          <h2 className="num-law"> 16.</h2>
          <p>
            <strong>Fuero y Competencia:</strong> Para cualquier cuestión
            judicial que pudiera derivarse de la realización de la Promoción,
            los Participantes y el Organizador se someterán a la jurisdicción de
            los Tribunales Ordinarios en lo Comercial con asiento en la Ciudad
            Autónoma de Buenos Aires.
          </p>
        </div>
        <article className="table-container mt-4  mb-5">
          <h2 className="num-law">Tabla de Puntos de Venta Adheridos</h2>
          <table className="table">
            <thead>
              <tr>
                <th>PUNTO DE VENTA ADHERIDO</th>
                <th>DIRECCION</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td>PATIO CERVECERIA SANTA FE</td>
                <td>Calchines 1398, Santa Fe</td>
              </tr>
              <tr>
                <td>DON MARCOS</td>
                <td>Castellanos esq. Las Heras, Santa Fe</td>
              </tr>
              <tr>
                <td>LA CITY</td>
                <td>San Martín y La Rioja, Santa Fe</td>
              </tr>
              <tr>
                <td>PRIMOS</td>
                <td>Av General López 2614, Santa Fe</td>
              </tr>
              <tr>
                <td>1980</td>
                <td>Costanera Este, Santa Fe</td>
              </tr>
              <tr>
                <td>El Palacio de Doña Rafaela</td>
                <td>Av. Aristóbulo del Valle 6729, Santa Fe</td>
              </tr>
              <tr>
                <td>BIZARRO</td>
                <td>Av. 7 de marzo 1498, Santo Tomé</td>
              </tr>
              <tr>
                <td>EL NACIONAL</td>
                <td>Ituzaingo y Dorrego, Santa Fe</td>
              </tr>
              <tr>
                <td>DIVA</td>
                <td>Alvear & Boulevard, Santa Fe</td>
              </tr>
              <tr>
                <td>LO DE NESTOR</td>
                <td>Av. Gral. Paz 6308, Santa Fe</td>
              </tr>
              <tr>
                <td>MITOS</td>
                <td>Sarmiento y Maipú, Santa Fe</td>
              </tr>
              <tr>
                <td>EL CLÁSICO</td>
                <td>Hipólito Irigoyen 3179</td>
              </tr>
              <tr>
                <td>SAYMON</td>
                <td>Gral. Paz 5785, Santa Fe</td>
              </tr>
              <tr>
                <td>CLUB INDEPENDIENTE</td>
                <td>Dr Zavalla 2860, Santa Fe</td>
              </tr>
              <tr>
                <td>ZULMA ALE</td>
                <td>General Paz 5201, Santa Fe</td>
              </tr>
              <tr>
                <td>El Alberdi, Alma de bodegón</td>
                <td>Belgrano 3660, Santa Fe</td>
              </tr>
            </tbody>
          </table>
        </article>
      </section>
    </Layout>
  );
};
